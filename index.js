    ...
    try {
        ...
        if (msg.content.trim().toLowerCase() == _CMD_JOIN) {
            if (!msg.member.voice.channelID) {
                msg.reply('Phải join vào voice channel trước.')
            } else {
                if (!guildMap.has(mapKey))
                    await connect(msg, mapKey)
                else
                    msg.reply('Đã join rồi.')
            }
            ...
            if (guildMap.has(mapKey)) {
                ...
                msg.reply("Không thể thoát vì chưa connect.")
            }
        }
...
async function connect(msg, mapKey) {
    try {
        ...
        if (!voice_Channel) return msg.reply("Voice channel không xuất hiện!");
        let text_Channel = await discordClient.channels.fetch(msg.channel.id);
        if (!text_Channel) return msg.reply("Text channel Không xuất hiện!");
